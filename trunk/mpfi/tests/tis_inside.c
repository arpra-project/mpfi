...
check ()
{
  ...
  for (i = 0; i <= 1000; ++i) {
    ...
    if (mpfi_is_inside (i1, i2) && !mpfr_equal_p (b, c)) {
      print_error (i1, i2);
    }
    if (mpfi_is_inside (i2, i1) && !mpfr_equal_p (b, c)) {
      print_error (i2, i1);
    }
    mpfr_set_nan (&(i1->right));
    if (mpfi_is_inside (i1, i2)) {
      print_error (i1, i2);
    }

    mpfi_interv_fr (i1, a, c);
    mpfi_interv_fr (i2, b, d);
    if (mpfi_is_inside (i1, i2) && !mpfr_equal_p (a, b)
        && !mpfr_equal_p (b, c) && !mpfr_equal_p (c, d)) {
      print_error (i1, i2);
    }
    if (mpfi_is_inside (i2, i1) && !mpfr_equal_p (a, b)
        && !mpfr_equal_p (b, c) && !mpfr_equal_p (c, d)) {
      print_error (i2, i1);
    }
    mpfr_set_nan (&(i1->right));
    ...
    if (!(mpfr_equal_p (a, b) && mpfr_equal_p (c, d))
        && mpfi_is_inside (i1, i2)) {
      print_error (i1, i2);
    }
    if (!mpfi_is_inside (i2, i1)) {
      print_error (i2, i1);
    }
    mpfr_set_nan (&(i1->right));
    if (mpfi_is_inside (i2, i1)) {
