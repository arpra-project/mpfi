...
check_overflow ()
{
  ...
  inexact = mpfi_div_fr (a, a, x);

  if (!mpfr_inf_p (&(a->right))) {
    printf ("Error: mpfi_div_fr does not correctly handle positive "
            "overflow.\n");
    exit (1);
  }

  if (!MPFI_RIGHT_IS_INEXACT (inexact)) {
    printf ("Error: mpfi_div_fr does not return correct value when positive "
            "overflow.\n");
    exit (1);
  }
  ...
  if (!mpfr_inf_p (&(a->left))) {
    printf ("Error: mpfi_div_fr does not correctly handle negative "
            "overflow.\n");
    exit (1);
  }

  if (!MPFI_LEFT_IS_INEXACT (inexact)) {
    printf ("Error: mpfi_div_fr does not return correct value when negative "
            "overflow.\n");
    exit (1);
  }
